"""
ROVER Baseline Evaluation Script — Realworld Dataset Format

Implements ROVER's recursive subtask decomposition approach (Schroeder et al., NeurIPS 2025)
adapted for pairwise frame comparison evaluation on realworld datasets that use
...
per-frame progress through those subtasks. We adapt this for pairwise comparison by:
  ...
  2. Scoring each frame independently with a 3-frame sliding window
     (initial frame, previous frame, current frame)
  3. Comparing scores to determine which frame shows more progress
...
python3 examples/scripts/myscripts/evaluate_ROVER_realworld.py \
...
Usage (Local Qwen):
CUDA_VISIBLE_DEVICES=0 python3 examples/scripts/myscripts/evaluate_ROVER_realworld.py \
...
python3 examples/scripts/myscripts/evaluate_ROVER_realworld.py \
    --model_name_or_path "gemini-3-flash-preview" \
    --base_dataset_path "realworld_dataset/TurnMugRightsideUp-Nominal-real" \
    --split val \
    --num_samples 100 \
    --batch_size 100 \
    --visualize
...
# ============================================================================
# Task token to natural language description
# ============================================================================
...
TASK_TOKEN_TO_DESC = {
    ...
    "[PUSH_COASTER_TO_MUG]": "Push the coaster toward the mug until they are adjacent",
    "[TURN_MUG_RIGHTSIDE_UP]": "Flip the upside-down mug so it is rightside up",
    "[BIKE_ROTOR_INSTALL]": "Install the bike rotor using both arms in a bimanual manipulation",
    "[CLEAR_KITCHEN_COUNTER]": "Clear the kitchen counter by moving all items to their designated locations using both arms",
    "[SETUP_BREAKFAST_TABLE]": "Set up the breakfast table by placing all items in their correct positions using both arms",
    "[CLEAN_LITTERBOX]": "Clean the litter box by scooping and disposing of waste",
    "[CUT_APPLE]": "Cut the apple into slices using the knife",
}
...
def run_inference_gemini(client, model_name, pairs, batch_size, device, subtask_cache, max_new_tokens=64):
    ...
    async def _score_pair(idx):
        ...
        try:
            ...
            initial1 = extract_frame(item["video_path_1"], 1)
            prev_idx1 = max(1, idx1 - SLIDING_WINDOW_LOOKBACK)
            previous1 = extract_frame(item["video_path_1"], prev_idx1)
            frame1 = extract_frame(item["video_path_1"], idx1)
        except Exception as e:
        ...
        try:
            ...
            initial2 = extract_frame(item["video_path_2"], 1)
            prev_idx2 = max(1, idx2 - SLIDING_WINDOW_LOOKBACK)
            previous2 = extract_frame(item["video_path_2"], prev_idx2)
            frame2 = extract_frame(item["video_path_2"], idx2)
        except Exception as e:
...
def run_inference_local(model, processor, pairs, batch_size, device, subtask_cache, max_new_tokens=256):
...
def _get_interval_label(r):
    """Classify a result by its compare_interval or as 'sf' (success-failure)."""
...
def visualize_results(results, output_dir, num_examples=8):
    ...
    fig.suptitle(f"ROVER Realworld Evaluation Results (n={len(valid)})", fontsize=14, fontweight="bold")
    ...
    for label, samples in [("worst", worst), ("best", best)]:
        ...
        for idx, r in enumerate(samples):
            ...
            interval_str = _get_interval_label(r)
            ...
            row_base = idx * 2
            try:
                initial1 = extract_frame(r["video_path_1"], 1)
                prev_idx1 = max(1, f1_idx - SLIDING_WINDOW_LOOKBACK) if isinstance(f1_idx, int) else 0
                previous1 = extract_frame(r["video_path_1"], prev_idx1)
                current1 = extract_frame(r["video_path_1"], f1_idx)
            ...
            try:
                initial2 = extract_frame(r["video_path_2"], 1)
                prev_idx2 = max(1, f2_idx - SLIDING_WINDOW_LOOKBACK) if isinstance(f2_idx, int) else 0
                previous2 = extract_frame(r["video_path_2"], prev_idx2)
                current2 = extract_frame(r["video_path_2"], f2_idx)
            ...
            # Shared super-label for this sample pair
        ...
        plt.suptitle(f"ROVER Realworld {label.capitalize()} Predictions (3-frame context)", fontsize=13, fontweight="bold", y=1.02)
        plt.tight_layout()
...
def parse_args():
    p = argparse.ArgumentParser(description="ROVER baseline evaluation on realworld datasets")
...
def main():
    ...
    root_output_dir = args.output_dir or os.path.join(
        ...
        f"{args.prefix}rover_eval_realworld_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    os.makedirs(root_output_dir, exist_ok=True)

    # ============================
    # Build ALL pairs across all tasks first (matching realworld eval pattern)
    # ============================
    all_pairs = []
    ...
    if args.num_samples and args.num_samples < len(pairs):
        ...
        group_order = sorted(groups.keys(),
                             key=lambda l: (0, 0) if l == "sf" else (1, int(l.split("-")[1]) if "-" in l else 0))
        first_pass = {}
        leftover_budget = 0
    ...
    if use_local:
        ...
        print(f"\nLoading local model for ROVER evaluation: {args.model_name_or_path}")
        ...
        print(f"\nInitializing Gemini Client for ROVER evaluation with model: {args.model_name_or_path}")
        client = genai.Client(api_key=os.environ.get("GOOGLE_API_KEY"))
        model = None
        processor = None

    # Subtask decomposition cache
    subtask_cache = {}
    ...
    print("\n" + "=" * 60)
    print("ROVER REALWORLD EVALUATION RESULTS")
    print("=" * 60)
