...
"""
Evaluation script for SFT VLM models trained with sft_vlm.py

Usage:
CUDA_VISIBLE_DEVICES=2 python3 examples/scripts/myscripts/evaluate_sft_vlm.py \
    ...
    --temperature 0.1 \
    --tolerance 10 \
    --model_name_or_path /workspace/hf_trl/trl/outputs/sep24/sft-Qwen2.5-VL-7B-Instruct_20250924_140336/checkpoint-2000 \
    --dataset_path OOD \
    --dataset_split all_demos \
    --enable_ordering_eval

CUDA_VISIBLE_DEVICES=0 python3 examples/scripts/myscripts/evaluate_sft_vlm.py \
    --base_model_name_or_path /workspace/cosmos-reason1/data/huggingface/transformers/Qwen2.5-VL-7B-Instruct \
    --batch_size 300 \
    --dtype bfloat16 \
    --max_prompt_length 2048 \
    ...
    --temperature 0.1 \
    --tolerance 10 \
    --model_name_or_path /workspace/hf_trl/trl/outputs/sep24/sft-Qwen2.5-VL-7B-Instruct_20250924_140342/checkpoint-1000 \
    --dataset_path ID \
...
# Default task description and prompts
...
class EvaluationArgs:
    ...
    use_quantization: bool = field(
        default=False,
        metadata={"help": "Use 4-bit quantization for model loading"}
    )
...
class SFTEvaluator:
    def __init__(self, args: EvaluationArgs):
        ...
        self.output_dir += '/eval_sft_' + datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    ...
    def load_model(self):
        """Load the trained SFT model and processor"""
        ...
        if self.args.model_name_or_path and os.path.exists(os.path.join(self.args.model_name_or_path, "adapter_config.json")):
            ...
            print(f"Loading full finetuned model from {self.args.model_name_or_path}")
            self.model = AutoModelForVision2Seq.from_pretrained(
                self.args.model_name_or_path,
                torch_dtype=dtype if dtype != "auto" else None,
                device_map="auto",
            ...
            # Load base model
            print(f"Loading base model from {self.args.base_model_name_or_path}")
            self.model = AutoModelForVision2Seq.from_pretrained(
    ...
    def evaluate_accuracy(self, completion: str, ground_truth: str) -> tuple:
        ...
        try:
            ...
            extracted_number = float(completion)
    ...
    def generate_completion(self, batch: List[Dict]) -> List[str]:
        ...
        for item in batch:
            # Check if model is Qwen-based and use qwen_vl_utils if available
            # model_name = self.args.model_name_or_path or self.args.base_model_name_or_path
        ...
        # Process inputs with proper handling for different model types
        if images_batch and isinstance(images_batch[0], list):
            # Qwen-style image inputs
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate SFT VLM model")
    ...
    parser.add_argument("--enable_ordering_eval", action="store_true", default=False,
    ...
    # Run evaluation
    evaluator = SFTEvaluator(eval_args)
