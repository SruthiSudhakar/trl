"""
Trainer for PnPRedLegoToBrownBowl pairwise progress comparison.

Layout assumed at --dataset_root:
    <kind>_<N>_<timestamp>.npy            # dict: trajectory, timestamps, frequency, image_paths
    <kind>_<N>_<timestamp>_frames/        # frame_000000.jpg ...
...
Eval is a demo-level holdout (default demos 17-20 in each kind). A
PairwiseSignAccuracyCallback runs at every eval step, calls model.generate,
parses the signed integer, and logs per-bucket sign accuracy.
"""
...
# Lego-specific prompts: two separate images, "first" vs "second".
LEGO_SYSTEM_PROMPT = (
...
def load_lego_demos(root: str, indices: list, kind: str) -> list:
    demos = []
    for n in indices:
        ...
        frames_dir = path[:-4] + "_frames"
        if not os.path.isdir(frames_dir):
            logger.warning(f"Frames dir missing: {frames_dir}, skipping")
            continue
        demos.append({
...
def build_pairs_for_demos(
    ...
    user_prompt = LEGO_USER_PROMPT_TEMPLATE.format(task_token=task_token)
    ...
    def emit(v1, f1, v2, f2, ans, kind, demo_id, bucket):
        out.append({
            ...
            "demo_success": kind,
            "bucket": bucket,
            "job_name": "PnPRedLegoToBrownBowl",
    ...
    for s in successes:
        ...
        for interval in intervals:
            ...
            for i1 in range(offset, sub_max - interval, train_step):
                ...
                v1, vf1, v2, vf2 = s["video_path"], f1, s["video_path"], f2
                if rng.random() < 0.5:
                    v1, vf1, v2, vf2 = v2, vf2, v1, vf1
                    ans = -ans
                emit(v1, vf1, v2, vf2, ans, "success", s["idx"], f"succ_vs_succ_int{interval}")
                count += 1
            logger.info(f"success_{s['idx']} interval={interval}: {count} pairs")
    ...
    for f in failures:
        ...
        for i in kept:
            ...
            ans = ANSWER_MAGNITUDE
            v1, vf1, v2, vf2 = f["video_path"], f_src, s["video_path"], s_src
            if rng.random() < 0.5:
                v1, vf1, v2, vf2 = v2, vf2, v1, vf1
                ans = -ans
            emit(v1, vf1, v2, vf2, ans, "failure", f["idx"], "fail_vs_succ")
        logger.info(
            f"failure_{f['idx']}: n_sub={n_sub}, k={k}, "
            f"last_range=[{last_indices[0]}..{last_indices[-1]}], emitted {len(kept)} pairs"
        )
...
@dataclass
class LegoArgs:
    dataset_root: str = "/proj/vondrick3/datasets/VLMjgd/PnPRedLegoToBrownBowl"
    failure_indices: str = "1-20"
    success_indices: str = "1-20"
    eval_failure_indices: str = "17-20"
    eval_success_indices: str = "17-20"
...
if __name__ == "__main__":
    parser = TrlParser((ScriptArguments, SFTConfig, ModelConfig, LegoArgs))
    script_args, training_args, model_args, lego = parser.parse_args_and_config()
    ...
    if lego.task_name not in TASK_TOKENS:
        raise ValueError(f"Unknown task '{lego.task_name}'. Add it to TASK_TOKENS.")
    task_token = TASK_TOKENS[lego.task_name]

    succ_idxs = parse_indices(lego.success_indices)
    fail_idxs = parse_indices(lego.failure_indices)
    eval_succ_set = set(parse_indices(lego.eval_success_indices))
    eval_fail_set = set(parse_indices(lego.eval_failure_indices))

    successes = load_lego_demos(lego.dataset_root, succ_idxs, "success")
    failures = load_lego_demos(lego.dataset_root, fail_idxs, "failure")
    ...
    train_pairs = build_pairs_for_demos(
        train_succ, train_fail, intervals, lego.train_sample_interval, task_token,
        lego.failure_last_frac, lego.failure_min_frames,
        rng=random.Random(42),
    )
    eval_pairs = build_pairs_for_demos(
        eval_succ, eval_fail, intervals, lego.train_sample_interval, task_token,
        lego.failure_last_frac, lego.failure_min_frames,
