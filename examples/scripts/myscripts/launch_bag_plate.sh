...
# Three categories: box (success), glass (failure), remote (failure).
...
#
# Then:
# bash examples/scripts/myscripts/launch_bag_plate.sh
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 accelerate launch \
    ...
    --output_dir "outputs/BagPlate_$(date +%Y%m%d_%H%M%S)" \
    --eval_strategy steps \
    --logging_steps 10 \
    --eval_steps 50 \
    --save_steps 50 \
